...
public class MovieSearchSettings extends SettingsPaneImpl {
	public static final String[] YES_NO_COMBO = {"Restore last filters on startup","Reset to default on startup"};
	public static final String[] DEFAULT_ACTIONS_COMBO = {"Open movie in player","Show Movie details"};
	public static final int[] DEFAULT_ACTIONS = {MovieItem.PLAY_ACTION,MovieItem.DETAILS_ACTION};
	public static final String[] PARENTAL_COMBO = {"Never","PG when parental guidence is needed For young children",
		"PG-13 when containing innapropriate material for pre-Teens" ,"Only for rated R movies ( containing Adult content ) "};

	JLabel maxItemsPerPageLab = new JLabel("Items per page : ");
	JComboBox<String> maxItemsPerPageCombo = new JComboBox<String>(Settings.MAX_RESULT_PER_SEARCH_COMBO);
	...
	JComboBox<String> defaultOperationCombo = new JComboBox<String>(DEFAULT_ACTIONS_COMBO);
	JLabel warnParentalLab = new JLabel("Parental guide");
	JComboBox<String> warnParentalCombo = new JComboBox<String>(PARENTAL_COMBO);
	
	JComponent[] components = {maxItemsPerPageLab,maxItemsPerPageCombo,remeberFiltersLab,remeberFiltersCombo,defaultOperationLab,
	...
	public void loadCurrentSettings() {
		// TODO Auto-generated method stub
		maxItemsPerPageCombo.setSelectedIndex(Settings.getMaxSearchItemsPerPage());
		defaultOperationCombo.setSelectedIndex(Settings.getDefaultEnterOperation() == DEFAULT_ACTIONS[0] ? 0  : 1);
		remeberFiltersCombo.setSelectedIndex(Settings.getRememberFiltersOnStartup());
	...
	public void ApplySettings() {
		// TODO Auto-generated method stub
		Settings.setMaxSearchItemsPerPage(maxItemsPerPageCombo.getSelectedIndex());
		Settings.setDefaultEnterOperation(DEFAULT_ACTIONS[defaultOperationCombo.getSelectedIndex()]);
		Settings.setRememberFiltersOnStartup(remeberFiltersCombo.getSelectedIndex());
